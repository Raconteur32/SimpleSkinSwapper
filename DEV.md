...
| `26.3`      | 26.3-rc-3 | 25 | pre-release target; `deps.minecraft` bumps to `26.3` at release, which also becomes the active/VCS version. ModMenu 21.0.0-beta.1 (declares 26.3-rc-1) runs in the dev runtime. The run config forces `SDL_VIDEODRIVER=x11` on Linux: the new SDL windowing fails EGL init on native Wayland (override by setting the var yourself) |
