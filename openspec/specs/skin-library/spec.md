...
The full-screen management screen that replaced the carousel: a vertical category tab strip beside a responsive card grid, dressed with the vanilla recipe-book art. Cards reorder across categories, rotate, open a detail overlay (rename, display name, model switch, delete, replay) and end with an add card opening the add-skin overlay.
