...
- [x] 3. Wire `detektAll` into `.github/workflows/build.yml` after the multi-version build; full `./gradlew build detektAll` green; update tasks and flag anything version-specific learned for the code-review skill
      LEARNING: zero divergence between the src tree and the 4 generated trees under the current ledger — the branches stay honest. Skill updated with the detektAll gate.
